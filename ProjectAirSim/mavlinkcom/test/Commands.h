...
class Command {
 public:
  ...
  virtual void HandleMessage(const MavLinkMessage& msg) { unused(msg); }
...
class GetParamsCommand : public Command {
 ...
 public:
  ...
  virtual void PrintHelp() {
    printf(
        "params [logfile]- fetches the PX4 parameters and prints them or "
...
class GetSetParamCommand : public Command {
 ...
 public:
  GetSetParamCommand() { this->Name = "param [set name value] [get name]"; }
  virtual bool Parse(const std::vector<std::string>& args);

  virtual void PrintHelp() {
    printf(
        "param [set name value] [get name]  -- get or set a parameter by name "
        "(use params to see the list).\n");
  }
...
class TakeOffCommand : public Command {
 ...
 public:
  TakeOffCommand() { this->Name = "takeoff alt"; }
  virtual bool Parse(const std::vector<std::string>& args);

  virtual void PrintHelp() {
    printf(
        "takeoff alt - take off to given altitude above ground (in meters).\n");
  }
...
class LoiterCommand : public Command {
 public:
  LoiterCommand() { this->Name = "loiter"; }
  virtual bool Parse(const std::vector<std::string>& args);

  virtual void PrintHelp() { printf("loiter  - stay at current position.\n"); }
...
class RequestImageCommand : public Command {
 public:
  RequestImageCommand() { this->Name = "req_img"; }
  virtual bool Parse(const std::vector<std::string>& args);

  virtual void PrintHelp() { printf("req_img  - request image.\n"); }
...
class MissionCommand : public Command {
 ...
 public:
  MissionCommand() { this->Name = "mission"; }
  virtual bool Parse(const std::vector<std::string>& args);
...
class CapabilitiesCommand : public Command {
 ...
 public:
  CapabilitiesCommand() { this->Name = "cap"; }
  virtual bool Parse(const std::vector<std::string>& args);
...
class StatusCommand : public Command {
 ...
 public:
  StatusCommand() { this->Name = "status"; }
  virtual bool Parse(const std::vector<std::string>& args);
...
class PositionCommand : public Command {
 ...
 public:
  PositionCommand() { this->Name = "pos [home]"; }
  virtual bool Parse(const std::vector<std::string>& args);

  virtual void PrintHelp() {
    printf(
        "pos [home] - print current local and global position and optionally "
...
class FakeGpsCommand : public Command {
 ...
 public:
  FakeGpsCommand() { this->Name = "fakegps [start|stop]"; }
  virtual bool Parse(const std::vector<std::string>& args);
...
class HilCommand : public Command {
 ...
 public:
  HilCommand() { this->Name = "hil [start|stop]"; }
  virtual bool Parse(const std::vector<std::string>& args);

  virtual void PrintHelp() {
    printf(
        "hil [start|stop] - start stop simple hil simulation mode to generate "
        "fake GPS input.\n");
  }
...
class SendImageCommand : public Command {
 ...
 public:
  SendImageCommand() { this->Name = "sendimage filename width height"; }
  ...
  virtual void PrintHelp() {
    printf(
        "sendimage filename width height - send the specified image to remote "
        "proxy.\n");
  }
...
class PlayLogCommand : public Command {
 public:
  PlayLogCommand() { this->Name = "playlog filename"; }
...
class DumpLogCommandsCommand : public Command {
 public:
  DumpLogCommandsCommand() { this->Name = "dumplogcommands log_folder"; }

  virtual bool Parse(const std::vector<std::string>& args);

  virtual void PrintHelp() {
    printf(
        "dumplogcommands log_folder - dump commands for .mavlink file found in "
        "log folder output in subfolder.\n");
  }

  virtual void Execute(std::shared_ptr<MavLinkVehicle> com);

 private:
  static void processLogCommands(MavLinkFileLog& log,
...
class GotoCommand : public Command {
 ...
 public:
  ...
  virtual void PrintHelp() {
    printf(
        "goto x y z [speed] - move the drone to local coordinates x,y,z (in "
        "meters).\n");
  }
  ...
  virtual void Move(float vx, float vy, float vz, float heading,
                    bool isYaw = true);

  // start moving using the given velocities in x&y but with fixed altitude.
  virtual void MoveAltHold(float vx, float vy, float z, float heading,
                           bool isYaw = true);
  ...
  virtual void OnLostOffboardControl() {
    printf(
        "### ERROR: user took over control of the drone, we lost offboard "
        "control\n");
...
class OrbitCommand : public GotoCommand {
 public:
  ...
  virtual void PrintHelp() {
    printf(
        "orbit radius [speed] - orbit the current location & altitude at the "
        "given radius (in meters) with optional speed (default 1m/s).\n");
  }
...
class RotateCommand : public GotoCommand {
 ...
 public:
  RotateCommand() { this->Name = "rotate [speed]"; }
  virtual bool Parse(const std::vector<std::string>& args);

  virtual void PrintHelp() {
    printf(
        "rotate [speed] - rotate the drone at current location with given "
        "degrees/s\n");
  }
...
class SquareCommand : public GotoCommand {
 public:
  ...
  virtual void PrintHelp() {
    printf(
        "square length [speed] - make a square of given length at given speed "
        "(default 0.2m/s).\n");
  }
...
class PidController {
 ...
 public:
  // set desired point.
  void setPoint(float newSetPoint, float kProportional, float kIntegral,
                float kDerivative) {
...
class WiggleCommand : public Command {
 ...
 public:
  WiggleCommand() { this->Name = "wiggle [distance] [angle] [direction]"; }
  virtual bool Parse(const std::vector<std::string>& args);
  ...
  virtual void PrintHelp() {
    ...
    printf(
        "    angle    - the amount of attitude to get us moving in the "
        "specified direction.\n");
...
class AltHoldCommand : public Command {
 ...
 public:
  AltHoldCommand() { this->Name = "hold"; }
  virtual bool Parse(const std::vector<std::string>& args);

  virtual void PrintHelp() {
    printf(
        "hold alt kp ki kd  - move to given altitude using given PID controls "
        "and hold there.\n");
  }
  virtual void Close();
  virtual void Execute(std::shared_ptr<MavLinkVehicle> com);
...
class FtpCommand : public Command {
 public:
  ...
  virtual void PrintHelp() {
    printf(
        "ftp command allows you to transfer files to/from the remote drone, if "
        "the drone supports the MAV_PROTOCOL_CAPABILITY_FTP.\n");
  }
 ...
 private:
  ...
  void startMonitor();
  void stopMonitor();
  enum FtpCommandEnum { none, list, cd, get, put, remove, mkdir, rmdir };
  FtpCommandEnum cmd;
  std::string source;
  std::string target;
  std::string cwd;
...
class NshCommand : public Command {
 public:
  NshCommand() { this->Name = "nsh - start NuttX Shell"; }
  virtual bool Parse(const std::vector<std::string>& args);

  virtual void PrintHelp() { printf("nsh - start NuttX Shell.\n"); }
...
class SetMessageIntervalCommand : public Command {
 public:
  SetMessageIntervalCommand() {
    this->Name =
        "SetMessageInterval msgid interval - turn on/off a given mavlink "
        "stream";
  }
  virtual bool Parse(const std::vector<std::string>& args);
  ...
  virtual void PrintHelp() {
    printf(
        ...
        "stream.\n");
    printf(
        "where the msgid is an integer from mavlink common.xml, and the "
        "interval is.\n");
    printf(
        "a frequency, the number of messages per second you'd like to "
        "receive.\n");
...
class WaitForAltitudeCommand : public Command {
 public:
  WaitForAltitudeCommand() { this->Name = "WaitForAltitude z dz dvz"; }
  virtual bool Parse(const std::vector<std::string>& args);

  virtual void PrintHelp() {
    printf(
        "WaitForAltitudeCommand z dz dvz - wait for drone to get with in dz of "
        "the specified local z, and settle down to given velocity delta "
        "(dvz).\n");
  }
