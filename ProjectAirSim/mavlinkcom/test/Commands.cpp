...
// you must call this method if you want HandleMessage to be called
// subsequently.
void Command::Execute(std::shared_ptr<MavLinkVehicle> com) {
  ...
  if (subscription == 0) {
    subscription = com->getConnection()->subscribe(
        [=](std::shared_ptr<MavLinkConnection> connection,
          ...
          try {
            ...
            Utils::log(Utils::stringf("### Runtime Error: %s", e.what()),
                       Utils::kLogLevelError);
          } catch (...) {
            Utils::log(Utils::stringf("### unhandled exception"),
                       Utils::kLogLevelError);
          }
        });
  }
...
Command* Command::create(const std::vector<std::string>& args) {
  if (all_commands_ == nullptr)
    throw std::runtime_error(
        "all_commands_ member must be set before calling Command::parseLine()");
  ...
  for (size_t i = 0; i < cmdTable.size(); i++) {
    ...
    if (command->Parse(args)) return command;
  }
...
bool TakeOffCommand::Parse(const std::vector<std::string>& args) {
  if (args.size() > 0) {
    ...
    if (cmd == "takeoff") {
      ...
      printf(
          "Please provide a positive number, meters above current altitude\n");
      return false;
    }
  }
  return false;
...
void TakeOffCommand::Execute(std::shared_ptr<MavLinkVehicle> com) {
  // subscribe to get subsequent messages so we can track our progress towards
...
void DumpLogCommandsCommand::processLogCommands(MavLinkFileLog& log,
  ...
  while (log.read(msg, log_timestamp)) {
    if (log_start_timestamp == 0) log_start_timestamp = log_timestamp;

    switch (msg.msgid) {
      case MavLinkStatustext::kMessageId: {
        MavLinkStatustext status_msg;
        status_msg.decode(msg);
        if (std::strstr(status_msg.text, kCommandLogPrefix) ==
            status_msg.text) {
          std::string line = std::string(status_msg.text)
                                 .substr(std::strlen(kCommandLogPrefix));
          command_start_timestamp = log_timestamp;
          x_start = Utils::nan<float>();
          ++command_index;

          std::string command_text_file_name = FileSystem::combine(
              out_folder,
              Utils::stringf("cmd_%03d_%s.txt", command_index, line.c_str()));
          std::ofstream command_text_file(command_text_file_name.c_str());
          command_text_file << line;
          command_text_file.close();

          std::string command_pose_file_name =
              command_text_file_name + ".pos.txt";
          if (pos_log_file.is_open()) pos_log_file.close();
      ...
      case MavLinkLocalPositionNed::kMessageId: {
        if (pos_log_file.is_open()) {
          ...
          // find relative position since command start so we can compare two
          // commands better
          if (std::isnan(x_start)) {
            ...
            pos_log_file << log_timestamp - command_start_timestamp << "\t"
                         << heading << "\t";
            pos_log_file << pos_msg.x - x_start << "\t" << pos_msg.y - y_start
                         << "\t" << pos_msg.z - z_start << std::endl;
          }
        }
        break;
      }
...
void DumpLogCommandsCommand::Execute(std::shared_ptr<MavLinkVehicle> com) {
  ...
  for (directory_iterator next(dirPath), end; next != end; ++next) {
    ...
    if (ext == ".mavlink") {
      auto out_folder = FileSystem::createDirectory(FileSystem::combine(
          log_folder_, path.filename().stem().generic_string()));
...
void SyncParamValue(std::shared_ptr<MavLinkVehicle> com,
...
void PlayLogCommand::Execute(std::shared_ptr<MavLinkVehicle> com) {
  ...
  bool armed = false;
  playback_timestamp = playback_start_timestamp =
  ...
  while (log_.read(msg, log_timestamp)) {
    ...
    // sync clocks all the time so that the yellow ribbon also plays back at the
    // right speed.
    auto current_timestamp = MavLinkFileLog::getTimeStamp();
    long logDuration = static_cast<long>(log_timestamp - log_start_timestamp);
    long realDuration =
        static_cast<long>(current_timestamp - playback_start_timestamp);
    ...
    if (waitMicros > 0) {
      ...
      // our clock fell behind somehow (debug breakpoint?) So fix it by moving
      // our start time forwards by this amount.
      playback_start_timestamp -= waitMicros;
    }
    ...
    switch (msg.msgid) {
      case MavLinkStatustext::kMessageId: {
        ...
        if (std::strstr(status_msg.text, kCommandLogPrefix) ==
          ...
          if (command == nullptr) {
            throw std::runtime_error(std::string("Command for line ") + line +
                                     " cannot be found");
          }
          printf("Executing %s\n", line.c_str());
          try {
...
void TakeOffCommand::HandleMessage(const MavLinkMessage& msg) {
  if (msg.msgid ==
      static_cast<uint8_t>(MavLinkMessageIds::MAVLINK_MSG_ID_GPS_RAW_INT)) {
    ...
    if (!reached && alt >= targetAlt - delta && alt <= targetAlt + delta) {
      ...
      printf("Target altitude reached at lat=%f, long=%f, alt=%f\n", lat, lon,
             ...
             static_cast<uint8_t>(
                 MavLinkMessageIds::MAVLINK_MSG_ID_EXTENDED_SYS_STATE)) {
    ...
    if (!offground &&
        status.landed_state ==
            static_cast<uint8_t>(MAV_LANDED_STATE::MAV_LANDED_STATE_IN_AIR)) {
...
void LandCommand::Execute(std::shared_ptr<MavLinkVehicle> com) {
  ...
  bool rc = false;
  if (com->land(state.global_est.heading, state.home.global_pos.lat,
                state.home.global_pos.lon, state.home.global_pos.alt)
          .wait(3000, &rc)) {
    if (rc) {
      printf("ok\n");
    } else {
...
void RequestImageCommand::HandleMessage(const MavLinkMessage& msg) {
  if (msg.msgid == static_cast<uint8_t>(
                       MavLinkMessageIds::MAVLINK_MSG_ID_ENCAPSULATED_DATA)) {
...
void PositionCommand::HandleMessage(const MavLinkMessage& message) {
  if (printGlobalosition &&
      message.msgid == MavLinkGlobalPositionInt::kMessageId) {
    ...
    if (this->setHome) {
      ...
      if (!this->vehicle->getVehicleState().controls.landed) {
        printf(
            "Cannot set home position because it is not in the 'landed' "
            "state\n");
      } else {
  ...
  if (printLocalPosition &&
      message.msgid == MavLinkLocalPositionNed::kMessageId) {
    ...
    if (cmd == "fakegps") {
      if (args.size() > 1) {
        ...
        if (arg == "start") {
          ...
          printf(
              "fakegps [start|stop] - start stop simple generation of fake GPS "
              "input.\n");
          return false;
        }
      }
...
void FakeGpsCommand::Execute(std::shared_ptr<MavLinkVehicle> mav) {
  // note: we do not reset com when Close() is called because we want to keep
  // running. user has to invoke "hil stop" to stop the hil thread.
  this->com = mav;
}
    ...
    if (cmd == "hil") {
      if (args.size() > 1) {
        ...
        if (arg == "start") {
          ...
          printf(
              "hil [start|stop] - start stop simple hil simulation mode to "
              "generate fake GPS input.\n");
          return false;
        }
      }
      return true;
...
void HilCommand::HilThread() {
  ...
  while (started) {
    ...
    if (slices == 10) {
      ...
      gps.vn = static_cast<int16_t>(addNoise(0, 0.1f) * 100);  // cm/s
      gps.vel =
          static_cast<uint16_t>(std::abs(addNoise(0, 0.1f) * 100));  // cm/s
      gps.cog = static_cast<int16_t>(addNoise(0, 0.3f) * 100);  // degrees * 100
      if (com != nullptr) {
        com->sendMessage(gps);
      }
...
void SendImageCommand::Execute(std::shared_ptr<MavLinkVehicle> com) {
  ...
  if (ext == ".png") {
    type = static_cast<uint8_t>(
        MAVLINK_DATA_STREAM_TYPE::MAVLINK_DATA_STREAM_IMG_PNG);
  } else if (ext == ".jpg") {
    type = static_cast<uint8_t>(
        MAVLINK_DATA_STREAM_TYPE::MAVLINK_DATA_STREAM_IMG_JPEG);
  } else if (ext == ".bmp") {
    type = static_cast<uint8_t>(
        MAVLINK_DATA_STREAM_TYPE::MAVLINK_DATA_STREAM_IMG_BMP);
  } else {
  ...
  stream.sendFrame(reinterpret_cast<uint8_t*>(sbuf.buffer),
                   static_cast<uint32_t>(len), width, height, type, 100);
}
...
void CapabilitiesCommand::Execute(std::shared_ptr<MavLinkVehicle> com) {
  ...
  if (com->getCapabilities().wait(10000, &ver)) {
    ...
    if ((ver.capabilities &
         static_cast<int>(
             MAV_PROTOCOL_CAPABILITY::MAV_PROTOCOL_CAPABILITY_FTP)) != 0) {
    ...
    if ((ver.capabilities &
         static_cast<int>(
             MAV_PROTOCOL_CAPABILITY::
                 MAV_PROTOCOL_CAPABILITY_SET_POSITION_TARGET_LOCAL_NED)) != 0) {
    ...
    if ((ver.capabilities &
         static_cast<int>(
             MAV_PROTOCOL_CAPABILITY::MAV_PROTOCOL_CAPABILITY_TERRAIN)) != 0) {
  ...
  if (args.size() > 0) {
    ...
    if (cmd == "goto") {
      if (args.size() >= 4) {
        ...
        if (tz > 0) {
          tz = -tz;  // probably mean negative, since we are using NED
                     // coordinates.
        }
        if (std::fabs(tx) > 100) {
...
void GotoCommand::TakeControl() {
  if (!this->requestedControl) {
    // control works better if we get about 50 of these per second (20ms
    // interval, if we can).
...
void GotoCommand::Close() {
  if (this->requestedControl && vehicle != nullptr &&
      vehicle->hasOffboardControl()) {
...
void GotoCommand::HandleMessage(const MavLinkMessage& message) {
  switch (message.msgid) {
    case MavLinkLocalPositionNed::
        kMessageId:  // MAVLINK_MSG_ID_LOCAL_POSITION_NED:
    {
      ...
      if (targetPosition) {
        ...
        channel->moveToLocalPosition(tx, ty, tz, is_yaw,
                                     static_cast<float>(theading * M_PI / 180));

        if (this->hasLocalPosition) {
          if (!targetReached && std::abs(x - tx) < nearDelta &&
              std::abs(y - ty) < nearDelta) {
            targetReached = true;
          }
          if (targetReached && !settled &&
              (fabs(this->vx) + std::abs(this->vy) + std::abs(this->vz) <
               almostStationery)) {
            settled = true;
        ...
        channel->moveByLocalVelocity(tvx, tvy, tvz, is_yaw,
                                     static_cast<float>(theading * M_PI / 180));
      } else if (targetVelocityAltHold) {
        // must send these regularly to keep offboard control.
        // integrate the heading so it is smoother.
        channel->moveByLocalVelocityWithAltHold(
            tvx, tvy, tz, is_yaw, static_cast<float>(theading * M_PI / 180));
      }
      break;
    }
...
void GotoCommand::HasLocalPosition() { Goto(tx, ty, tz, -1, yaw); }

void GotoCommand::Goto(float targetX, float targetY, float targetZ, float speed,
                       float heading, bool isYaw) {
  targetPosition = true;
...
void GotoCommand::Move(float targetvx, float targetvy, float targetvz,
                       float heading, bool isYaw) {
...
void GotoCommand::MoveAltHold(float targetvx, float targetvy, float targetZ,
                              float heading, bool isYaw) {
  targetPosition = false;
  targetVelocity = false;
...
void OrbitCommand::Execute(std::shared_ptr<MavLinkVehicle> com) {
  if (!com->isLocalControlSupported()) {
    throw std::runtime_error(Utils::stringf(
        "Your drone does not support the "
        "MAV_PROTOCOL_CAPABILITY_SET_POSITION_TARGET_LOCAL_NED capability."));
  }
...
void OrbitCommand::UpdateTarget() {
  ...
  if (flyingToRadius) {
    ...
    MeasureTime(degrees);
    float heading =
        degrees + 180;  // point the camera towards the center of the circle.
    ...
    float tangent =
        static_cast<float>(angle + M_PI_2 - (correction * M_PI / 180));
    ...
    if (orbitSpeed < speed) {
      ...
      if (ms > 5000) {
        ...
        double percentage =
            static_cast<double>(ms) / 5000.0;  // 5 second target
        orbitSpeed = static_cast<float>(percentage * speed);
        // printf("speeding up to %f\n", orbitSpeed);
      }
    }

    MoveAltHold(static_cast<float>(newvx), static_cast<float>(newvy), cz,
                heading);

    if (logViewer.get() != nullptr) {
      // monitor the sin curves so we can see how on track or off track it
      // actually is. the shape of the curve will also tell us if we are
      // progressing at a consistent speed, the more deformed the sin curve the
      // worse our progress.

      // pack this tracking info into mavlink_vicon_position_estimate_t just
      // because we can....

      MavLinkViconPositionEstimate est;
      est.pitch = est.roll = est.z = est.yaw = 0;
      est.usec = static_cast<uint32_t>(
  ...
  if (args.size() > 0) {
    ...
    if (cmd == "rotate") {
      if (args.size() > 1) {
        ...
        // this enables us to test what happens when offboard control is lost
        // and resumed.
...
void RotateCommand::HasLocalPosition() {
  if (!start_) {
    // in case we are flying fast, we first do a Goto to get to a fixed
    // stationary position before we try and start rotating.
    Goto(x, y, z, -1, yaw);
  }
}
...
void WiggleCommand::Execute(std::shared_ptr<MavLinkVehicle> com) {
  if (!com->isAttitudeControlSupported()) {
    throw std::runtime_error(Utils::stringf(
        "Your drone does not support the "
        "MAV_PROTOCOL_CAPABILITY_SET_ATTITUDE_TARGET capability."));
  }
  started_ = false;
  ...
  com->setMessageInterval(
      static_cast<int>(MavLinkMessageIds::MAVLINK_MSG_ID_LOCAL_POSITION_NED),
      30);
  com->setMessageInterval(
      static_cast<int>(MavLinkMessageIds::MAVLINK_MSG_ID_ATTITUDE_TARGET), 30);
  ...
  switch (message.msgid) {
    case MavLinkAttitudeTarget::kMessageId: {
      ...
      if (!ready_) {
        ...
        // these PID values were calculated experimentally using AltHoldCommand,
        // this provides the best control over thrust to achieve minimal
        // over/under shoot in a reasonable amount of time.
...
void WiggleCommand::wiggleX(const MavLinkLocalPositionNed& pos) {
  ...
  // try and keep y on target by using roll, but only use a little bit since it
  // shouldn't wander too much in that direction.
  roll = fmax(-0.2f,
              fmin(0.2f, static_cast<float>((-pos.vy / 5.0f) + (dy / 10.0f))));

  // DebugOutput("ctrl=%f, sz=%f, z=%f, dz=%f, new thrust=%f", ctrl, sz_, z, sz_
  // - z, thrust);
...
void WiggleCommand::wiggleY(const MavLinkLocalPositionNed& pos) {
  ...
  // try and keep x on target by using pitch, but only use a little bit since it
  // shouldn't wander too much in that direction.
  pitch = fmax(-0.2f,
               fmin(0.2f, static_cast<float>((pos.vx / 5.0f) - (dx / 10.0f))));
...
void AltHoldCommand::Execute(std::shared_ptr<MavLinkVehicle> com) {
  ...
  com->moveToLocalPosition(pos.x, pos.y, pos.z, true,
                           static_cast<float>(state.attitude.yaw * M_PI / 180));
...
void AltHoldCommand::HandleMessage(const MavLinkMessage& message) {
  ...
  switch (message.msgid) {
    ...
    case MavLinkLocalPositionNed::kMessageId: {
      ...
      // try and keep x & y on target by using pitch & roll, but only use a
      // little bit since it shouldn't wander too much in that direction.
      ...
      // DebugOutput("ctrl=%f, sz=%f, z=%f, dz=%f, new thrust=%f", ctrl, sz_, z,
      // sz_ - z, thrust);
      vehicle->moveByAttitude(roll, pitch, 0, 0, 0, 0, thrust);
      break;
    }
    default:
      break;
...
std::string normalize(std::string arg) {
  if (FileSystem::kPathSeparator == '\\') {
    return replaceAll(arg, '/',
                      '\\');  // make sure user input matches what FileSystem
                              // will do when resolving paths.
  }
  return arg;
}
...
void FtpCommand::doList() {
  ...
  if (progress.average_rate != 0) {
    printf("%d msgs received, %f milliseconds per packet, longest delay=%f\n",
           progress.message_count, progress.average_rate,
           progress.longest_delay);
  }
}
...
void FtpCommand::doGet() {
  ...
  if (!parse(leaf, wildcards)) {
    ...
    printf("getting all files in: %s matching '%s' ", dir.c_str(),
           leaf.c_str());
      ...
      if (!info.is_directory) {
        if (matches(leaf, info.name)) {
          ...
          std::string targetFile =
              FileSystem::combine(normalize(dir), info.name);
          startMonitor();
...
void FtpCommand::monitor() {
  ...
  while (!progress.complete) {
    if (progress.goal > 0) {
      double p = static_cast<double>(progress.current) /
                 static_cast<double>(progress.goal);
...
bool FtpCommand::matches(const std::string& pattern,
                         const std::string& name) const {
  size_t p = 0;
  size_t ps = pattern.size();
  // we only support simple matching for now, we can add full regex later if we
  // need it.
  for (size_t i = 0, s = name.size(); i < s; i++) {
    if (p >= ps) {
      return false;
...
void NshCommand::send(std::string& msg) {
  ...
  ctrl.flags =
      static_cast<uint8_t>(SERIAL_CONTROL_FLAG::SERIAL_CONTROL_FLAG_RESPOND) |
...
void NshCommand::HandleMessage(const MavLinkMessage& msg) {
  if (msg.msgid ==
      static_cast<uint8_t>(MavLinkMessageIds::MAVLINK_MSG_ID_SERIAL_CONTROL)) {
    MavLinkSerialControl ctrl;
    ctrl.decode(msg);
    int len = ctrl.count;
    if (len > 0) {
      if (len > 3 && ctrl.data[len - 1] == 'K' && ctrl.data[len - 2] == '[' &&
          ctrl.data[len - 3] == '\x1b') {
