...
static const int pixhawkVendorId =
    9900;  ///< Vendor ID for Pixhawk board (V2 and V1) and PX4 Flow
static const int pixhawkFMUV4ProductId =
    18;  ///< Product ID for Pixhawk V2 board
static const int pixhawkFMUV2ProductId =
    17;  ///< Product ID for Pixhawk V2 board
static const int pixhawkFMUV2OldBootloaderProductId =
    22;  ///< Product ID for Bootloader on older Pixhawk V2 boards
// static const int pixhawkFMUV1ProductId = 16;     ///< Product ID for PX4 FMU
// V1 board

#define MAV_AUTOPILOT_ENUM_END \
  (static_cast<uint8_t>(MAV_AUTOPILOT::MAV_AUTOPILOT_ASLUAV) + 1)
#define MAV_TYPE_ENUM_END (static_cast<uint8_t>(MAV_TYPE::MAV_TYPE_ADSB) + 1)
#define MAV_STATE_ENUM_END \
  (static_cast<uint8_t>(MAV_STATE::MAV_STATE_POWEROFF) + 1)
...
__attribute__((__format__(__printf__, 1, 0))) void DebugOutput(
    const char* message, ...) {
  va_list args;
  va_start(args, message);
...
// The remote app is connected to Pixhawk, and is also "serving" UDP packets,
// this tells us what remote connection to create to talke to that server.
bool offboard = false;
PortAddress offboardEndPoint;
#define DEFAULT_OFFBOARD_PORT 14560

// this is used if you want to connect MavLinkTest to the serial port of the
// Pixhawk directly
bool serial = false;
std::string comPort;
int baudRate = 115200;

// server mode is when you want another app to connect to Pixhawk and publish
// data back to this process. this server will be listening for UDP packets,
// this is mutually exclusive with 'offboard' as this server will become the
// primary "droneConnection".  For example, jMAVSim can talk to this server
...
// These are used to echo the mavlink messages to other 3rd party apps like QGC
// or LogViewer.
std::vector<PortAddress> proxyEndPoints;
#define DEFAULT_PROXY_PORT 14580

// this switch controls whether we turn off the RC remote active link loss
// detection if you do not have radio connected this is needed to stop
// "failsafe" override in pixhawk from kicking in when you try and fly.
bool noRadio = false;
bool unitTest = false;
bool verbose = false;
bool nsh = false;
...
void OpenLogFiles() {
  if (logDirectory.size() > 0) {
    ...
    std::string today = Utils::stringf("%04d-%02d-%02d", local->tm_year + 1900,
    ...
    std::string input =
        Utils::stringf("%02d-%02d-%02d-input.%s", local->tm_hour, local->tm_min,
                       local->tm_sec, ext);
    ...
    std::string output =
        Utils::stringf("%02d-%02d-%02d-output.%s", local->tm_hour,
                       local->tm_min, local->tm_sec, ext);
...
const static FlagName MavSysSensorFlags[] = {
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_3D_GYRO),
     "MAV_SYS_STATUS_SENSOR_3D_GYRO - 0x01 3D gyro"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_3D_ACCEL),
     "MAV_SYS_STATUS_SENSOR_3D_ACCEL - 0x02 3D accelerometer"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_3D_MAG),
    ...
    {static_cast<int>(
     ...
     "MAV_SYS_STATUS_SENSOR_DIFFERENTIAL_PRESSURE - 0x10 differential "
     "pressure"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_GPS),
    ...
    {static_cast<int>(
     ...
     "MAV_SYS_STATUS_SENSOR_EXTERNAL_GROUND_TRUTH - 0x200 external ground "
     "truth (Vicon or Leica)"},
    {static_cast<int>(
         MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_ANGULAR_RATE_CONTROL),
     "MAV_SYS_STATUS_SENSOR_ANGULAR_RATE_CONTROL - 0x400 3D angular rate "
     "control"},
    {static_cast<int>(
         MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_ATTITUDE_STABILIZATION),
     "MAV_SYS_STATUS_SENSOR_ATTITUDE_STABILIZATION - 0x800 attitude "
     "stabilization"},
    {static_cast<int>(
    ...
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_RC_RECEIVER),
     "MAV_SYS_STATUS_SENSOR_RC_RECEIVER - 0x10000 rc receiver"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_3D_GYRO2),
     "MAV_SYS_STATUS_SENSOR_3D_GYRO2 - 0x20000 2nd 3D gyro"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_3D_ACCEL2),
     "MAV_SYS_STATUS_SENSOR_3D_ACCEL2 - 0x40000 2nd 3D accelerometer"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_SENSOR_3D_MAG2),
     "MAV_SYS_STATUS_SENSOR_3D_MAG2 - 0x80000 2nd 3D magnetometer"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_GEOFENCE),
     "MAV_SYS_STATUS_GEOFENCE - 0x100000 geofence"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_AHRS),
     "MAV_SYS_STATUS_AHRS - 0x200000 AHRS subsystem health"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_TERRAIN),
     "MAV_SYS_STATUS_TERRAIN - 0x400000 Terrain subsystem health"},
    {static_cast<int>(MAV_SYS_STATUS_SENSOR::MAV_SYS_STATUS_REVERSE_MOTOR),
...
const static char* AutoPilotNames[]{
    ...
    "MAV_AUTOPILOT_GENERIC_WAYPOINTS_ONLY, Generic autopilot only supporting "
    "simple waypoints",
    "MAV_AUTOPILOT_GENERIC_WAYPOINTS_AND_SIMPLE_NAVIGATION_ONLY, Generic "
    "autopilot supporting waypoints and other simple navigation commands",
    "MAV_AUTOPILOT_GENERIC_MISSION_FULL, Generic autopilot supporting the full "
    "mission command set",
    "MAV_AUTOPILOT_INVALID,  No valid autopilot, e.g. a GCS or other MAVLink "
    "component",
...
const static char* MavTypeNames[]{
    ...
    "MAV_TYPE_VTOL_DUOROTOR-  Two-rotor VTOL using control surfaces in "
    "vertical operation in addition. Tailsitter.",
    "MAV_TYPE_VTOL_QUADROTOR-  Quad-rotor VTOL using a V-shaped quad config in "
    "vertical operation. Tailsitter.",
...
const static FlagName ModeFlagNames[]{
    {static_cast<int>(MAV_MODE_FLAG::MAV_MODE_FLAG_TEST_ENABLED),
     "MAV_MODE_FLAG_TEST_ENABLED - system has a test mode enabled. This flag "
    ...
    {static_cast<int>(MAV_MODE_FLAG::MAV_MODE_FLAG_AUTO_ENABLED),
     "MAV_MODE_FLAG_AUTO_ENABLED - autonomous mode enabled, system finds its "
     "own goal positions. Guided flag can be set or not, depends on the actual "
     "implementation."},
    {static_cast<int>(MAV_MODE_FLAG::MAV_MODE_FLAG_GUIDED_ENABLED),
     "MAV_MODE_FLAG_GUIDED_ENABLED - guided mode enabled, system flies "
     "MISSIONs / mission items."},
    {static_cast<int>(MAV_MODE_FLAG::MAV_MODE_FLAG_STABILIZE_ENABLED),
     "MAV_MODE_FLAG_STABILIZE_ENABLED - system stabilizes electronically its "
     "attitude (and optionally position). It needs however further control "
     "inputs to move around."},
    {static_cast<int>(MAV_MODE_FLAG::MAV_MODE_FLAG_HIL_ENABLED),
     "MAV_MODE_FLAG_HIL_ENABLED - hardware in the loop simulation. All motors "
     "/ actuators are blocked, but internal software is full operational."},
    {static_cast<int>(MAV_MODE_FLAG::MAV_MODE_FLAG_MANUAL_INPUT_ENABLED),
     "MAV_MODE_FLAG_MANUAL_INPUT_ENABLED - remote control input is enabled."},
    {static_cast<int>(MAV_MODE_FLAG::MAV_MODE_FLAG_SAFETY_ARMED),
     "MAV_MODE_FLAG_SAFETY_ARMED - MAV safety set to armed. Motors are enabled "
...
const static char* MavStateNames[]{
    ...
    "MAV_STATE_STANDBY - System is grounded and on standby. It can be launched "
    "any time.",
    "MAV_STATE_ACTIVE - System is active and might be already airborne. Motors "
    "are engaged.",
    "MAV_STATE_CRITICAL - System is in a non-normal flight mode. It can "
    "however still navigate.",
    "MAV_STATE_EMERGENCY - System is in a non-normal flight mode. It lost "
    "control over parts or over the whole airframe. It is in mayday and going "
    "down.",
    "MAV_STATE_POWEROFF - System just initialized its power-down sequence, "
    "will shut down now.",
    "MAV_STATE_ENUM_END - ",
};
...
void PrintFlags(const FlagName* flagNames, int value) {
  for (int i = 0;; i++) {
    if (flagNames[i].Name == NULL) break;
...
void PrintEnum(const FlagName* enumNames, int value) {
  for (int i = 0;; i++) {
    if (enumNames[i].Name == NULL) break;
...
void PrintCustomMode(const MavLinkHeartbeat& heartbeat) {
  if (heartbeat.autopilot ==
      static_cast<uint8_t>(MAV_AUTOPILOT::MAV_AUTOPILOT_PX4)) {
...
void mavlink_dcm_to_euler(const float dcm[3][3], float* roll, float* pitch,
                          float* yaw) {
  float phi, theta, psi;
  theta = asin(-dcm[2][0]);
...
void mavlink_quaternion_to_euler(const float quaternion[4], float* roll,
...
extern void mavlink_euler_to_quaternion(float roll, float pitch, float yaw,
                                        float quaternion[4]);
...
void PrintUsage() {
  ...
  printf("Options: \n");
  printf(
      "    -udp:ipaddr[:port]]                    - connect to remote drone "
      "via this udp address, the remote app is the UDP server (default port "
      "14550)\n");
  printf(
      "    -server:ipaddr[:port]                  - start mavlink server on "
      "this local port (so jMAVSim can connect to it using -udp)\n");
  printf("    -serial[:comPortName][, baudrate]]     - open serial port\n");
  printf(
      "    -logviewer:ipaddr[:port]               - for sending mavlink "
      "information to Log Viewer\n");
  printf(
      "    -proxy:ipaddr[:port]                   - send all mavlink messages "
      "to and from remote node\n");
  printf(
      "    -local:ipaddr                          - specify local NIC address "
      "(default 127.0.0.1)\n");
  printf(
      "    -logdir:filename                       - specify local directory "
      "where mavlink logs are stored (default is no log files)\n");
  printf(
      "    -logformat:json                        - the default is binary "
      ".mavlink, if you specify this option you will get mavlink logs in "
      "json\n");
  printf(
      "    -convert:[json,csv]                    - convert all existing "
      ".mavlink log files in the logdir to the specified -logformat\n");
  printf(
      "    -filter:msid,msgid,...                 - while converting .mavlink "
      "log extract only the given mavlink message ids\n");
  printf(
      "    -noradio							   - "
      "disables RC link loss failsafe\n");
  printf(
      "    -nsh                                   - enter NuttX shell "
      "immediately on connecting with PX4\n");
  printf(
      "    -telemetry                             - generate telemetry mavlink "
      "messages for logviewer\n");
  printf(
      "    -wifi:iface                            - add wifi rssi to the "
      "telemetry using given wifi interface name (e.g. wplsp0)\n");
  printf("If no arguments it will find a COM port matching the name 'PX4'\n");
  printf(
      "You can specify -proxy multiple times with different port numbers to "
      "proxy drone messages out to multiple listeners\n");
}
    ...
    if (arg[0] == '-' || arg[0] == '/') {
      ...
      if (lower == "udp") {
        ...
        if (parts.size() > 1) {
          ...
          for (auto ptr = fparts.begin(), end = fparts.end(); ptr != end;
               ptr++) {
            std::string f = *ptr;
            try {
              long ft = std::stol(f);
      ...
      } else if (lower == "h" || lower == "?" || lower == "help" ||
                 lower == "-help") {
        return false;
...
std::shared_ptr<MavLinkConnection> connectProxy(const PortAddress& endPoint,
                                                std::string name) {
  printf("Connecting to UDP Proxy address %s:%d\n", endPoint.addr.c_str(),
         endPoint.port);

  std::shared_ptr<MavLinkConnection> proxyConnection =
      MavLinkConnection::connectRemoteUdp(name, defaultLocalAddress,
                                          endPoint.addr, endPoint.port);

  // forward all PX4 messages to the remote proxy and all messages from remote
  // proxy to PX4.
...
std::string findPixhawk() {
  ...
  for (auto iter = result.begin(); iter != result.end(); iter++) {
    ...
    if (info.vid == pixhawkVendorId) {
      if (info.pid == pixhawkFMUV4ProductId ||
...
std::shared_ptr<MavLinkConnection> connectSerial() {
  ...
  return MavLinkConnection::connectSerial("drone", name, baudRate,
...
std::shared_ptr<MavLinkConnection> connectOffboard() {
  ...
  return MavLinkConnection::connectRemoteUdp("drone", defaultLocalAddress,
                                             offboardEndPoint.addr,
                                             offboardEndPoint.port);
}

std::shared_ptr<MavLinkConnection> connectServer(const PortAddress& endPoint,
                                                 std::string name) {
  printf("Connecting to UDP Server address %s:%d\n", endPoint.addr.c_str(),
         endPoint.port);

  std::shared_ptr<MavLinkConnection> serverConnection =
      MavLinkConnection::connectLocalUdp(name, endPoint.addr, endPoint.port);
...
bool connect() {
  if (offboard && serial) {
    printf(
        "Cannot connect to local -serial pixhawk and -udp drone at the same "
        "time \n");
    return false;
  }
  if (!offboard && !serial && !server) {
    printf(
        "Must specify one of -serial, -udp or -server otherwise we don't have "
        "a drone connection\n");
    return false;
  }
  if (offboard && server) {
    printf(
        "Cannot have offboard and server, must pick one of this as the primary "
        "drone connection\n");
    return false;
  }
  ...
  if (server) {
    ...
    std::shared_ptr<MavLinkConnection> serverConnection =
        connectServer(serverEndPoint, "server");
    usedPorts.push_back(serverEndPoint);

    if (droneConnection != nullptr) {
      // then we have a serial connection as the primary droneConnection, so
      // publish all PX4 messages out to the server
  ...
  if (verbose) {
    droneConnection->subscribe(
        [=](std::shared_ptr<MavLinkConnection> con, const MavLinkMessage& msg) {
  ...
  if (!server) {
    ...
    mavLinkVehicle->startHeartbeat(heartbeat);
  }
  ...
  for (auto ptr = proxyEndPoints.begin(), end = proxyEndPoints.end();
       ptr != end; ptr++) {
    PortAddress proxyEndPoint = *ptr;
    for (auto ep = usedPorts.begin(), endep = usedPorts.end(); ep != endep;
         ep++) {
      PortAddress used = *ep;
...
void checkPulse() {
  ...
  if (!mavLinkVehicle->waitForHeartbeat().wait(100000, &heartbeat)) {
    throw std::runtime_error(
        "Received no heartbeat from PX4 after 100 seconds");
  }
}

const char* IgnoreStateTable[] = {"Baro #0 fail:  STALE!", nullptr};
...
void handleStatus(const MavLinkStatustext& statustext) {
  ...
  Utils::log(Utils::stringf("STATUS: sev=%d, '%s'\n",
                            static_cast<int>(statustext.severity),
                            safeText.c_str()));
}
...
int console(std::stringstream& script) {
  std::string line;
  mavLinkVehicle =
      std::make_shared<MavLinkVehicle>(LocalSystemId, LocalComponentId);
  ...
  // this is advanced command that can get us into trouble on real drone, so
  // remove it for now.
  ...
  droneConnection->subscribe([=](std::shared_ptr<MavLinkConnection> connection,
    ...
    switch (message.msgid) {
      ...
      case MavLinkAttitudeTarget::kMessageId:
        ...
        // DebugOutput("q1 : %f\t%f\t%f\t%g", target.q[0], target.q[1],
        // target.q[2], target.q[3]); DebugOutput("q2 : %f\t%f\t%f\t%g", q2[0],
        // q2[1], q2[2], q2[3]); DebugOutput("target roll: %f\tpitch:
        // %f\tyaw:%f\tthrust: %f", roll, pitch, yaw, target.thrust);
        break;
  ...
  if (logConnection != nullptr) {
    logViewer =
        std::make_shared<MavLinkNode>(LocalLogViewerSystemId, LocalComponentId);
  ...
  if (nsh) {
    ...
    if (!noparams) {
      printf(
          "Downloading drone parameters so we know how to control it "
          "properly...\n");
      try {
  ...
  while (!std::cin.eof()) {
    ...
    if (line.length() > 0) {
      ...
      if (cmd == "x") {
        ...
        if (selected != nullptr && inLogFile != nullptr &&
            inLogFile->isOpen()) {
...
int main(int argc, const char* argv[]) {
  ...
  if (serial) {
    if (comPort.size() == 0 || comPort == "*") {
      ...
      if (comPort == "") {
        printf(
            "### Error: PX4 not found on your SerialPort, or it is not "
            "available");
        return 1;
      }
    }
