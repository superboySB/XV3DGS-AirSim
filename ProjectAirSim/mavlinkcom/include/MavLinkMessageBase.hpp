...
// This is the raw undecoded message, use the msgid field to figure out which
// strongly typed MavLinkMessageBase subclass can be used to decode this
// message.  For example, a heartbeat: MavLinkMessage msg; if (msg.msgid ==
...
// This is the base class for all the strongly typed messages define in
// MavLinkMessages.hpp
class MavLinkMessageBase {
 public:
  ...
  // find what type of message this is and decode it on the heap (call delete
 ...
 protected:
  ...
  void pack_char_array(int len, char* buffer, const char* field,
                       int offset) const;
  void pack_uint8_t_array(int len, char* buffer, const uint8_t* field,
                          int offset) const;
  void pack_int8_t_array(int len, char* buffer, const int8_t* field,
                         int offset) const;
  void pack_uint16_t_array(int len, char* buffer, const uint16_t* field,
                           int offset) const;
  void pack_int16_t_array(int len, char* buffer, const int16_t* field,
                          int offset) const;
  void pack_uint32_t_array(int len, char* buffer, const uint32_t* field,
                           int offset) const;
  ...
  void unpack_uint8_t_array(int len, const char* buffer, uint8_t* field,
                            int offset);
  void unpack_int8_t_array(int len, const char* buffer, int8_t* field,
                           int offset);
  void unpack_uint16_t_array(int len, const char* buffer, uint16_t* field,
                             int offset);
  void unpack_int16_t_array(int len, const char* buffer, int16_t* field,
                            int offset);
  ...
  std::string uint32_t_array_tostring(int len, const uint32_t* field);
  std::string int32_t_array_tostring(int len, const int32_t* field);
  std::string float_array_tostring(int len, const float* field);
  std::string float_tostring(float value);
  std::string double_array_tostring(int len, const double* field);
  std::string double_tostring(double value);
};

// Base class for all strongly typed MavLinkCommand classes defined in
// MavLinkMessages.hpp
...
class MavLinkTelemetry : public MavLinkMessageBase {
 public:
  ...
  uint32_t messages_sent =
      0;  // number of messages sent since the last telemetry message
  uint32_t messages_received =
      0;  // number of messages received since the last telemetry message
  uint32_t messages_handled =
      0;  // number of messages handled since the last telemetry message
  uint32_t crc_errors = 0;  // # crc errors detected in mavlink stream since the
                            // last telemetry message
  uint32_t handler_microseconds =
      0;  // total time spent in the handlers in microseconds since the last
          // telemetry message
  uint32_t render_time =
      0;  // total time spent rendering frames since the last telemetry message
  int32_t wifi_rssi = 0;  // if this device is communicating over wifi this is
                          // the signal strength.
  uint32_t update_rate =
      0;  // rate at which update() is being called on MavLinkMultiRotorApi
  uint32_t actuation_delay =
      0;  // delay from HIL_SENSOR to HIL_ACTUATORCONTROLS response
  ...
  uint32_t update_time =
      0;  // time inside MavLinkMultiRotorApi::update() method

  // not serialized
  const char* wifiInterfaceName =
      nullptr;  // the name of the wifi interface we are measuring RSSI on.
