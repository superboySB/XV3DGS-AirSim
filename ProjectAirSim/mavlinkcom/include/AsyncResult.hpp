...
class AsyncResultState {
 public:
  ...
  AsyncResultState(CompletionHandler owner) { owner_ = owner; }

  ~AsyncResultState() { complete(); }
...
class AsyncResult {
 public:
  ...
  int getState() { return state_->getState(); }

  void setState(int s) const { state_->setState(s); }
  ...
  AsyncResult(const AsyncResult<T>& other) { this->state_ = other.state_; }
  ...
  bool isCompleted() { return state_->isCompleted(); }
