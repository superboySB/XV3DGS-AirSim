...
typedef struct _VehicleState {
  ...
  struct AttitudeState {
    float roll = 0, pitch = 0, yaw = 0, roll_rate = 0, yaw_rate = 0,
          pitch_rate = 0;
  ...
  struct LocalState {
    Vector3
        pos;  // in NEU (north, east, up) coordinates (positive Z goes upwards).
    Vector3 lin_vel;
    Vector3 acc;
    uint64_t updated_on;
  } local_est;
  ...
  struct HomeState {
    ...
    Vector3 approach;  // in NEU (north, east, up) coordinates (positive Z goes
                       // upwards).
    bool is_set = false;
  } home;
