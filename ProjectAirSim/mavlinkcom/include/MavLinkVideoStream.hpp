...
class MavLinkVideoClient : public MavLinkNode {
 public:
  ...
  struct MavLinkVideoFrame {
    ...
    float progress;  ///< while frame is being assembled this returns the
...
class MavLinkVideoServer : public MavLinkNode {
 public:
  ...
  // poll this function to see if there is a valid image request, it returns
  // false if there is no request.
  bool hasVideoRequest(MavLinkVideoRequest& req);

  // call this to send the image back over the connection given to start
  // function.
