...
class MavLinkNode {
 public:
  ...
  // already doing it.
  void startHeartbeat(const MavLinkHeartbeat& heartbeat);
  ...
  // needs changing, change the value, then call setParameter with the same
  // parameter object.
  ...
  // request that target node sends this message stream at given frequency
  // (specify messages per second that you want). where msgId is a
  // MAVLINK_MSG_ID_* enum value.
  void setMessageInterval(int msgId, int frequency);

  // wait a given amount of time for a heart beat, and return the decoded
  // message.
  ...
  // Send raw undecoded messge to remote node (this is handy in proxy scenarios
  // where you are simply passing a message along).
  void sendMessage(MavLinkMessage& msg);

  // send a command to the remote node
  void sendCommand(MavLinkCommand& cmd);

  // send a command to the remote node and get async result that tells you
