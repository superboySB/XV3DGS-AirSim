...
std::shared_ptr<MavLinkConnection> MavLinkConnection::connectSerial(
    const std::string& nodeName, const std::string& portName, int baudrate,
    const std::string& initString) {
  return MavLinkConnectionImpl::connectSerial(nodeName, portName, baudrate,
                                              initString);
}

std::shared_ptr<MavLinkConnection> MavLinkConnection::connectLocalUdp(
...
std::shared_ptr<MavLinkConnection> MavLinkConnection::connectRemoteUdp(
    const std::string& nodeName, const std::string& localAddr,
    const std::string& remoteAddr, int remotePort) {
  return MavLinkConnectionImpl::connectRemoteUdp(nodeName, localAddr,
                                                 remoteAddr, remotePort);
}

std::shared_ptr<MavLinkConnection> MavLinkConnection::connectTcp(
    const std::string& nodeName, const std::string& localAddr,
    const std::string& remoteIpAddr, int remotePort) {
  return MavLinkConnectionImpl::connectTcp(nodeName, localAddr, remoteIpAddr,
                                           remotePort);
}
...
void MavLinkConnection::startLoggingSendMessage(
...
void MavLinkConnection::startLoggingReceiveMessage(
    std::shared_ptr<MavLinkLog> log) {
...
void MavLinkConnection::close() { pImpl->close(); }

bool MavLinkConnection::isOpen() { return pImpl->isOpen(); }
...
std::string MavLinkConnection::getName() { return pImpl->getName(); }
...
void MavLinkConnection::unsubscribe(int id) { pImpl->unsubscribe(id); }
...
void MavLinkConnection::join(std::shared_ptr<MavLinkConnection> remote,
...
// whatever timeslice you decide to call this method.
...
// MavLinkConnection& MavLinkConnection::operator=(MavLinkConnection&&) =
// default;
