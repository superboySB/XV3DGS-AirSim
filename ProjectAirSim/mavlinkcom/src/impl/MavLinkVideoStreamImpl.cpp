...
MavLinkVideoClientImpl::MavLinkVideoClientImpl(int localSystemId,
                                               int localComponentId)
    : MavLinkNodeImpl(localSystemId, localComponentId) {}

MavLinkVideoClientImpl::~MavLinkVideoClientImpl() { close(); }

void MavLinkVideoClientImpl::handleMessage(
    std::shared_ptr<MavLinkConnection> connection,
    const MavLinkMessage& message) {
  unused(connection);
  switch (message.msgid) {
    case MavLinkDataTransmissionHandshake::
        kMessageId:  // MAVLINK_MSG_ID_DATA_TRANSMISSION_HANDSHAKE:
    {
      ...
      incoming_image.start = static_cast<uint32_t>(
    ...
    case MavLinkEncapsulatedData::
        kMessageId:  // MAVLINK_MSG_ID_ENCAPSULATED_DATA:
    {
      ...
      if (incoming_image.packets == 0) {
        incoming_image.packetsArrived = 0;
        Utils::log("Aborting image reception because of zero packets",
                   Utils::kLogLevelWarn);
        break;
      }
      ...
      if (incoming_image.packetsArrived >= incoming_image.packets) {
        Utils::log(Utils::stringf("Image is available: %d packets arrived",
                                  incoming_image.packetsArrived));
...
bool MavLinkVideoClientImpl::readNextFrame(
    MavLinkVideoClient::MavLinkVideoFrame& image) {
  return incoming_image.read(image);
}

// image APIs
void MavLinkVideoClientImpl::requestVideo(int camera_id, float every_n_sec,
                                          bool save_locally) {
...
void MavLinkVideoServerImpl::handleMessage(
    std::shared_ptr<MavLinkConnection> connection,
    const MavLinkMessage& message) {
  unused(connection);
  switch (message.msgid) {
...
bool MavLinkVideoServerImpl::hasVideoRequest(
...
void MavLinkVideoServerImpl::sendFrame(uint8_t data[], uint32_t data_size,
