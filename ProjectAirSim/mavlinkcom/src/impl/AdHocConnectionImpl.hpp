...
class AdHocConnectionImpl {
 public:
  AdHocConnectionImpl();
  static std::shared_ptr<AdHocConnection> connectSerial(
      const std::string& nodeName, std::string portName, int baudrate = 115200,
      const std::string initString = "");
  static std::shared_ptr<AdHocConnection> connectLocalUdp(
      const std::string& nodeName, std::string localAddr, int localPort);
  static std::shared_ptr<AdHocConnection> connectRemoteUdp(
      const std::string& nodeName, std::string localAddr,
      std::string remoteAddr, int remotePort);
  static std::shared_ptr<AdHocConnection> connectTcp(
      const std::string& nodeName, std::string localAddr,
  ...
  ~AdHocConnectionImpl();
  void startListening(std::shared_ptr<AdHocConnection> parent,
 ...
 private:
  static std::shared_ptr<AdHocConnection> createConnection(
