...
// <Cfgmgr32.h> must be included after <ObjIdl.h>
#include <Cfgmgr32.h>
#include <SetupAPI.h>
#include <Windows.h>
#include <propkey.h>
...
const GUID serialDeviceClass = {0x4d36e978, 0xe325, 0x11ce, 0xBF, 0xC1, 0x08,
...
std::vector<SerialPortInfo> MavLinkConnection::findSerialPorts(int vid,
  ...
  HDEVINFO classInfo = SetupDiGetClassDevsEx(&serialDeviceClass, NULL, NULL,
  ...
  for (int devIndex = 0;
       SetupDiEnumDeviceInfo(classInfo, devIndex, &deviceInfo); devIndex++) {
    ULONG size;
    HRESULT hr = CM_Get_Device_ID_Size(&size, deviceInfo.DevInst, 0);
    if (hr == CR_SUCCESS) {
      std::wstring buffer(size + 1, '\0');
      hr = CM_Get_Device_ID(deviceInfo.DevInst, (PWSTR)buffer.c_str(), size + 1,
                            0);
      ...
      if (parseVidPid(buffer, &dvid, &dpid) &&
        ...
        DWORD keyCount = 0;
        if (!SetupDiGetDevicePropertyKeys(classInfo, &deviceInfo, NULL, 0,
                                          &keyCount, 0)) {
        ...
        if (SetupDiGetDevicePropertyKeys(classInfo, &deviceInfo, keyArray,
                                         keyCount, &keyCount, 0)) {
          for (DWORD j = 0; j < keyCount; j++) {
            DEVPROPKEY* key = &keyArray[j];
            bool isItemNameProperty =
                (key->fmtid == PKEY_ItemNameDisplay.fmtid &&
                 key->pid == PKEY_ItemNameDisplay.pid);
            if (isItemNameProperty) {
              ULONG bufferSize = 0;
              DEVPROPTYPE propertyType;
              CM_Get_DevNode_Property(deviceInfo.DevInst, &keyArray[j],
                                      &propertyType, NULL, &bufferSize, 0);
              if (bufferSize > 0) {
                BYTE* propertyBuffer = new BYTE[bufferSize];
                hr = CM_Get_DevNode_Property(deviceInfo.DevInst, &keyArray[j],
