...
// already doing it.
void MavLinkNodeImpl::startHeartbeat(const MavLinkHeartbeat& heartbeat) {
...
void MavLinkNodeImpl::sendHeartbeat() {
  ...
  while (heartbeat_running_) {
    ...
    std::this_thread::sleep_for(
        std::chrono::milliseconds(heartbeatMilliseconds));
  }
}

// this is called for all messages received on the connection.
void MavLinkNodeImpl::handleMessage(
  ...
  // this is for the subclasses to play with.  We put nothing here so we are not
  // dependent on the subclasses remembering to call this base implementation.
}
...
AsyncResult<MavLinkAutopilotVersion> MavLinkNodeImpl::getCapabilities() {
  if (has_cap_) {
    AsyncResult<MavLinkAutopilotVersion> nowait(
        [=](int state) { unused(state); });
  ...
  AsyncResult<MavLinkAutopilotVersion> result(
      [=](int state) { con->unsubscribe(state); });
...
AsyncResult<MavLinkHeartbeat> MavLinkNodeImpl::waitForHeartbeat() {
  ...
  // wait for a heartbeat msg since this will give us the port to send commands
  // to...
  // this->setMessageInterval(static_cast<int>(MavLinkMessageIds::MAVLINK_MSG_ID_HEARTBEAT),
  // 1);
  auto con = ensureConnection();

  AsyncResult<MavLinkHeartbeat> result(
      [=](int state) { con->unsubscribe(state); });
  ...
  int subscription =
      con->subscribe([=](std::shared_ptr<MavLinkConnection> connection,
        ...
        unused(connection);
        if (m.msgid ==
            static_cast<uint8_t>(MavLinkMessageIds::MAVLINK_MSG_ID_HEARTBEAT)) {
...
void MavLinkNodeImpl::sendOneHeartbeat() {
  ...
  try {
    sendMessage(heartbeat_);
  } catch (std::exception& e) {
    // ignore any failures here because we are running in our own thread here.
    Utils::log(Utils::stringf(
        "Caught and ignoring exception sending heartbeat: %s", e.what()));
  } catch (...) {
    // ignore any failures here because we are running in our own thread here.
    Utils::log(Utils::stringf(
        "Caught and ignoring unknown exception sending heartbeat."));
  }
}
...
float UnpackParameter(uint8_t type, float param_value) {
  ...
  switch (static_cast<MAV_PARAM_TYPE>(type)) {
    ...
    case MAV_PARAM_TYPE::MAV_PARAM_TYPE_UINT64:
      // we only have 4 bytes for the value in mavlink_param_value_t, so how
      // does this one work?
      value = static_cast<float>(pu.ui);
      break;
...
void MavLinkNodeImpl::assertNotPublishingThread() {
  ...
  if (con->isPublishThread()) {
    throw std::runtime_error(
        "Cannot perform blocking operation on the connection publish thread");
  }
}
...
std::vector<MavLinkParameter> MavLinkNodeImpl::getParamList() {
  ...
  while (!done) {
    ...
    if (!paramReceived.timed_wait(3000)) {
      // timeout, so we'll drop through to the code below which will try and fix
      // this...
      done = true;
    }
    waiting = false;
  ...
  // note that UDP does not guarantee delivery of messages, so we have to also
  // check if some parameters are missing and get them individually.
  std::vector<size_t> missing;

  for (size_t i = 0; i < paramCount; i++) {
    // nested loop is inefficient, but it is needed because UDP also doesn't
  ...
  for (auto iter = missing.begin(), end = missing.end(); iter != end; iter++) {
    ...
    if (getParameterByIndex(static_cast<int16_t>(*iter)).wait(2000, &r)) {
      ...
      Utils::log(Utils::stringf("Paremter %d does not seem to exist", index),
                 Utils::kLogLevelWarn);
    }
  }

  std::sort(result.begin(), result.end(),
            [&](const MavLinkParameter& p1, const MavLinkParameter& p2) {
...
MavLinkParameter MavLinkNodeImpl::getCachedParameter(const std::string& name) {
  if (this->parameters_.size() == 0) {
    throw std::runtime_error(
        ...
        "cached snapshot of the parameter values");
  }
  ...
  throw std::runtime_error(
      Utils::stringf("Error: parameter name '%s' not found"));
}

AsyncResult<MavLinkParameter> MavLinkNodeImpl::getParameter(
    const std::string& name) {
  int size = static_cast<int>(name.size());
  if (size > 16) {
    throw std::runtime_error(Utils::stringf(
        "Error: parameter name '%s' is too long, must be <= 16 chars",
        name.c_str()));
  } else if (size < 16) {
    size++;  // we can include the null terminator.
  }
  auto con = ensureConnection();
  AsyncResult<MavLinkParameter> asyncResult(
      [=](int state) { con->unsubscribe(state); });
...
AsyncResult<MavLinkParameter> MavLinkNodeImpl::getParameterByIndex(
    int16_t index) {
  auto con = ensureConnection();
  AsyncResult<MavLinkParameter> asyncResult(
      [=](int state) { con->unsubscribe(state); });
...
AsyncResult<bool> MavLinkNodeImpl::setParameter(MavLinkParameter p) {
  ...
  auto con = ensureConnection();
  assertNotPublishingThread();
  AsyncResult<bool> result([=](int state) { con->unsubscribe(state); });
  bool gotit = false;
  MavLinkParameter q;
  for (size_t i = 0; i < 10; i++) {
    Utils::log(Utils::stringf(
                   "setParameter() calling getParameter() for %s, try #%d...\n",
                   p.name.c_str(), i + 1),
               Utils::kLogLevelInfo);
    if (getParameter(p.name).wait(1000, &q)) {
      gotit = true;
      break;
    }
  }
  if (!gotit) {
    throw std::runtime_error(Utils::stringf(
        "Error: parameter name '%s' was not found", p.name.c_str()));
  }
...
AsyncResult<bool> MavLinkNodeImpl::sendCommandAndWaitForAck(
    MavLinkCommand& command) {
  auto con = ensureConnection();
        ...
        if (message.msgid == MavLinkCommandAck::kMessageId) {
          ...
          if (ack.command == cmd) {
            ...
            if (ackResult == MAV_RESULT::MAV_RESULT_TEMPORARILY_REJECTED) {
              Utils::log(Utils::stringf(
                  "### command %d result: MAV_RESULT_TEMPORARILY_REJECTED",
                  cmd));
              ...
              Utils::log(Utils::stringf("### command %d unexpected result: %d",
                                        cmd, ackResult));
            }
