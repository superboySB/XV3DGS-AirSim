...
class MavLinkVideoClientImpl : public MavLinkNodeImpl {
 public:
  ...
  // or if you are implementing the client side call this function to get the
  // most recent frame. returns false if there is no new frame available.
 ...
 private:
  ...
  struct IncomingFrame {
    ...
    int payload;  ///< Payload size per transmitted packet (bytes). Standard is
                  ///< 254, and decreases when image resolution increases.
    int quality;  ///< Quality of the transmitted image (percentage)
    int type;     ///< Type of the transmitted image (BMP, PNG, JPEG, RAW 8 bit,
                  ///< RAW 32 bit)
    ...
    bool read(MavLinkVideoClient::MavLinkVideoFrame& image) {
      ...
      if (is_ready && is_unread) {
        ...
        image.progress =
            static_cast<float>(packetsArrived) / static_cast<float>(packets);
   ...
   private:
    bool is_unread =
        false;  // was inage data returned by getReceievedImage() call
    bool is_ready = false;
...
class MavLinkVideoServerImpl : public MavLinkNodeImpl {
 public:
  ...
  void sendFrame(uint8_t data[], uint32_t data_size, uint16_t width,
                 uint16_t height, uint8_t image_type, uint8_t image_quality);
