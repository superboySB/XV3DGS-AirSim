...
class MavLinkFtpClientImpl : public MavLinkNodeImpl {
 public:
  ...
  bool isSupported();
  void list(MavLinkFtpProgress& progress, const std::string& remotePath,
            std::vector<MavLinkFileInfo>& files);
  void get(MavLinkFtpProgress& progress, const std::string& remotePath,
           const std::string& localPath);
  void put(MavLinkFtpProgress& progress, const std::string& remotePath,
           const std::string& localPath);
 ...
 private:
  ...
  // the following state is needed to implement a restartable state machine for
  // each command with a watchdog
  enum FtpCommandEnum {
    FtpCommandNone,
    FtpCommandList,
    FtpCommandGet,
