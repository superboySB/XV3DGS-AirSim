...
std::vector<SerialPortInfo> MavLinkConnection::findSerialPorts(int vid,
  ...
  // todo: alternative:  probably need to do an lstat on '/dev/serial/by-id' and
  // find something that looks like PX4 and return that name, or follow the
  // symbolic link to /dev/ttyACM0...

  std::string bash_command =
      "bash -c '"
      "for sysdevpath in $(find /sys/bus/usb/devices/usb*/ -name dev); do "
      "syspath=\"${sysdevpath%/dev}\"; devname=\"$(udevadm info -q name -p "
      "$syspath)\"; "
