...
std::shared_ptr<AdHocConnection> AdHocConnectionImpl::createConnection(
...
std::shared_ptr<AdHocConnection> AdHocConnectionImpl::connectLocalUdp(
    const std::string& nodeName, std::string localAddr, int localPort) {
...
std::shared_ptr<AdHocConnection> AdHocConnectionImpl::connectRemoteUdp(
    const std::string& nodeName, std::string localAddr, std::string remoteAddr,
    int remotePort) {
...
std::shared_ptr<AdHocConnection> AdHocConnectionImpl::connectTcp(
    const std::string& nodeName, std::string localAddr,
...
std::shared_ptr<AdHocConnection> AdHocConnectionImpl::connectSerial(
    const std::string& nodeName, std::string name, int baudRate,
  ...
  if (hr != 0)
    throw std::runtime_error(Utils::stringf(
        "Could not open the serial port %s, error=%d", name.c_str(), hr));
...
void AdHocConnectionImpl::startListening(
    std::shared_ptr<AdHocConnection> parent, const std::string& nodeName,
...
void AdHocConnectionImpl::sendMessage(const std::vector<uint8_t>& msg) {
  ...
  try {
    ...
    throw std::runtime_error(
        Utils::stringf("AdHocConnectionImpl: Error sending message on "
                       "connection '%s', details: %s",
                       name.c_str(), e.what()));
  } catch (...) {
    throw std::runtime_error(Utils::stringf(
        "AdHocConnectionImpl: Unknown exception while sending message on "
        "connection '%s'.",
        name.c_str()));
  }
}
  ...
  while (hr == 0 && con_ != nullptr && !closed) {
    ...
    if (count >= MAXBUFFER) {
      std::cerr << "GAH KM911 message size (" << std::to_string(count)
                << ") is bigger than max buffer size! Time to support frame "
                   "breaks, Moffitt"
                << std::endl;
...
void AdHocConnectionImpl::drainQueue() {
  ...
  while (hasMsg) {
    ...
    std::shared_ptr<AdHocConnection> sharedPtr =
        std::shared_ptr<AdHocConnection>(this->con_);
    for (auto ptr = snapshot.begin(); ptr != end; ptr++) {
      try {
        (*ptr).handler(sharedPtr, message);
      } catch (std::exception& e) {
        Utils::log(Utils::stringf("AdHocConnectionImpl: Error handling message "
                                  "on connection '%s', details: %s",
                                  name.c_str(), e.what()),
                   Utils::kLogLevelError);
      } catch (...) {
        Utils::log(
            Utils::stringf(
                "AdHocConnectionImpl: Unknown exception while handling message "
                "on connection '%s'.",
                name.c_str()),
            Utils::kLogLevelError);
      }
