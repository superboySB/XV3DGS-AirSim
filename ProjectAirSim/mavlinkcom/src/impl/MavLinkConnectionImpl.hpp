...
class MavLinkConnectionImpl {
 public:
  ...
  ~MavLinkConnectionImpl();
  void startListening(std::shared_ptr<MavLinkConnection> parent,
 ...
 private:
  static std::shared_ptr<MavLinkConnection> createConnection(
      const std::string& nodeName, std::shared_ptr<Port> port);
  void joinLeftSubscriber(std::shared_ptr<MavLinkConnection> remote,
                          std::shared_ptr<MavLinkConnection> con,
                          const MavLinkMessage& msg);
  void joinRightSubscriber(std::shared_ptr<MavLinkConnection> con,
                           const MavLinkMessage& msg);
  void publishPackets();
  void readPackets();
  void drainQueue();
  ...
  uint8_t message_buf[300];  // must be bigger than sizeof(mavlink_message_t),
                             // which is currently 292.
