...
std::string MavLinkConnectionImpl::getName() { return name; }
...
std::shared_ptr<MavLinkConnection> MavLinkConnectionImpl::createConnection(
    const std::string& nodeName, std::shared_ptr<Port> port) {
  // std::shared_ptr<MavLinkCom> owner, const std::string& nodeName
  std::shared_ptr<MavLinkConnection> con =
      std::make_shared<MavLinkConnection>();
  con->startListening(nodeName, port);
  return con;
}

std::shared_ptr<MavLinkConnection> MavLinkConnectionImpl::connectLocalUdp(
...
std::shared_ptr<MavLinkConnection> MavLinkConnectionImpl::connectRemoteUdp(
    const std::string& nodeName, const std::string& localAddr,
    const std::string& remoteAddr, int remotePort) {
  std::string local = localAddr;
  // just a little sanity check on the local address, if remoteAddr is localhost
  // then localAddr must be also.
...
std::shared_ptr<MavLinkConnection> MavLinkConnectionImpl::connectTcp(
    const std::string& nodeName, const std::string& localAddr,
...
std::string MavLinkConnectionImpl::acceptTcp(
    std::shared_ptr<MavLinkConnection> parent, const std::string& nodeName,
    const std::string& localAddr, int listeningPort) {
  std::string local = localAddr;
  close();
  std::shared_ptr<TcpClientPort> socket = std::make_shared<TcpClientPort>();

  port = socket;  // this is so that a call to close() can cancel this blocking
                  // accept call.
...
std::shared_ptr<MavLinkConnection> MavLinkConnectionImpl::connectSerial(
    const std::string& nodeName, const std::string& portName, int baudRate,
  ...
  if (hr != 0)
    throw std::runtime_error(Utils::stringf(
        "Could not open the serial port %s, error=%d", portName.c_str(), hr));

  // send this right away just in case serial link is not already configured
  if (initString.size() > 0) {
    serial->write(reinterpret_cast<const uint8_t*>(initString.c_str()),
                  static_cast<int>(initString.size()));
  }

  return createConnection(nodeName, serial);
}

void MavLinkConnectionImpl::startListening(
    std::shared_ptr<MavLinkConnection> parent, const std::string& nodeName,
...
void MavLinkConnectionImpl::startLoggingSendMessage(
    std::shared_ptr<MavLinkLog> log) {
  sendLog_ = log;
}

void MavLinkConnectionImpl::stopLoggingSendMessage() { sendLog_ = nullptr; }

// log every message that is "sent" using sendMessage.
void MavLinkConnectionImpl::startLoggingReceiveMessage(
    std::shared_ptr<MavLinkLog> log) {
  receiveLog_ = log;
}
...
void MavLinkConnectionImpl::sendMessage(const MavLinkMessage& m) {
  if (ignored_messageids.find(m.msgid) != ignored_messageids.end()) return;
...

  {
    ...
    try {
      ...
      throw std::runtime_error(
          Utils::stringf("MavLinkConnectionImpl: Error sending message on "
                         "connection '%s', details: %s",
                         name.c_str(), e.what()));
    } catch (...) {
      throw std::runtime_error(Utils::stringf(
          "MavLinkConnectionImpl: Unknown exception while sending message on "
          "connection '%s'.",
          name.c_str()));
    }
  }
...
int MavLinkConnectionImpl::prepareForSending(MavLinkMessage& msg) {
  ...
  bool signing =
      !mavlink1 && mavlink_status_.signing &&
      (mavlink_status_.signing->flags & MAVLINK_SIGNING_FLAG_SIGN_OUTGOING);
  ...
  if (len != msglen) {
    if (mavlink1) {
      ...
      // mavlink2 supports trimming the payload of trailing zeros so the
      // messages are variable length as a result.
    }
  }
  ...
  if (signing) {
    mavlink_sign_packet(
        mavlink_status_.signing, reinterpret_cast<uint8_t*>(msg.signature),
...
void MavLinkConnectionImpl::joinLeftSubscriber(
    std::shared_ptr<MavLinkConnection> remote,
...
void MavLinkConnectionImpl::joinRightSubscriber(
    std::shared_ptr<MavLinkConnection> connection, const MavLinkMessage& msg) {
  unused(connection);
...
void MavLinkConnectionImpl::join(std::shared_ptr<MavLinkConnection> remote,
                                 bool subscribeToLeft, bool subscribeToRight) {
  if (subscribeToLeft)
    this->subscribe(std::bind(&MavLinkConnectionImpl::joinLeftSubscriber, this,
                              remote, std::placeholders::_1,
                              std::placeholders::_2));

  if (subscribeToRight)
    remote->subscribe(std::bind(&MavLinkConnectionImpl::joinRightSubscriber,
...
void MavLinkConnectionImpl::readPackets() {
  ...
  while (hr == 0 && con_ != nullptr && !closed) {
    ...
    if (count <= 0) {
      // error? well let's try again, but we should be careful not to spin too
      // fast and kill the CPU
      std::this_thread::sleep_for(std::chrono::milliseconds(1));
      continue;
    }
    for (int i = 0; i < count; i++) {
      uint8_t frame_state =
          mavlink_frame_char_buffer(&msgBuffer, &mavlink_intermediate_status_,
                                    buffer[i], &msg, &mavlink_status_);
      ...
      if (frame_state == MAVLINK_FRAMING_INCOMPLETE) {
        ...
        if (mavlink_intermediate_status_.flags &
        ...
        if (con_ != nullptr && !closed) {
          ...
          // queue event for publishing.
          {
            ...
            ::memcpy(message.signature, msg.signature, 13);
            ::memcpy(message.payload64, msg.payload64,
...
void MavLinkConnectionImpl::drainQueue() {
  ...
  while (hasMsg) {
    ...
    // publish the message from this thread, this is safer than publishing from
    // the readPackets thread as it ensures we don't lose messages if the
    // listener is slow.
    if (snapshot_stale) {
      // this is tricky, the clear has to be done outside the lock because it is
      // destructing the handlers and the handler might try and call
      // unsubscribe, which needs to be able to grab the lock, otherwise we
    ...
    if (message.msgid ==
        static_cast<uint8_t>(
            MavLinkMessageIds::MAVLINK_MSG_ID_AUTOPILOT_VERSION)) {
    ...
    std::shared_ptr<MavLinkConnection> sharedPtr =
        std::shared_ptr<MavLinkConnection>(this->con_);
    for (auto ptr = snapshot.begin(); ptr != end; ptr++) {
      try {
        (*ptr).handler(sharedPtr, message);
      } catch (std::exception& e) {
        Utils::log(Utils::stringf("MavLinkConnectionImpl: Error handling "
                                  "message %d on connection '%s', details: %s",
                                  message.msgid, name.c_str(), e.what()),
                   Utils::kLogLevelError);
      } catch (...) {
        Utils::log(
            Utils::stringf("MavLinkConnectionImpl: Unknown exception handling "
                           "message %d on connection '%s'.",
                           message.msgid, name.c_str()),
            Utils::kLogLevelError);
      }
    }

    {
      ...
      long microseconds = static_cast<long>(
