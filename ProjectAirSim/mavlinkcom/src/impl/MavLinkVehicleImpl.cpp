...
#define MAVLINK_MSG_SET_POSITION_TARGET_IGNORE_POSITION \
  0x7  // if 0x7 bits are on the PX4 ignores position.
#define MAVLINK_MSG_SET_POSITION_TARGET_ALT_HOLD \
  0x3  // if 0x4 bit can be used to do z altitude hold.
#define MAVLINK_MSG_SET_POSITION_TARGET_IGNORE_VELOCITY \
  0x38  // if 0x38 bits are on on the PX4 ignores velocity.
#define MAVLINK_MSG_SET_POSITION_TARGET_IGNORE_ACCELERATION \
  0x1C0  // if 0x1C0 bits are on the PX4 ignores acceleration.
#define MAVLINK_MSG_SET_POSITION_TARGET_FORCE \
  0x200  // if 0x200 is on the PX4 interprets acceleration as a force.
#define MAVLINK_MSG_SET_POSITION_TARGET_IGNORE_YAW_ANGLE \
  0x400  // if 0x400 is on the PX4 ignores the yaw
#define MAVLINK_MSG_SET_POSITION_TARGET_IGNORE_YAW_RATE \
  0x800  // if 0x800 is on the PX4 ignores the yaw rate.
...
AsyncResult<MavLinkHomePosition> MavLinkVehicleImpl::waitForHomePosition() {
  ...
  AsyncResult<MavLinkHomePosition> result(
      [=](int subscription) { con->unsubscribe(subscription); });

  this->setMessageInterval(
      static_cast<int>(MavLinkMessageIds::MAVLINK_MSG_ID_HOME_POSITION), 1);

  int subscription =
      con->subscribe([=](std::shared_ptr<MavLinkConnection> connection,
                         const MavLinkMessage& m) {
        unused(connection);
        if (m.msgid == static_cast<uint8_t>(
                           MavLinkMessageIds::MAVLINK_MSG_ID_HOME_POSITION)) {
...
void MavLinkVehicleImpl::handleMessage(
    std::shared_ptr<MavLinkConnection> connection, const MavLinkMessage& msg) {
  MavLinkNodeImpl::handleMessage(connection, msg);

  // status messages should usually be only sent by actual PX4. However if
  // someone else is sending it to, we should listen it. in future it would be
  // good to have ability to add system IDs we are interested in if (msg.sysid
  // != getTargetSystemId())
  //{
  ...
  switch (msg.msgid) {
    case MavLinkHeartbeat::kMessageId: {  // MAVLINK_MSG_ID_HEARTBEAT:
      ...
      bool armed =
          (heartbeat.base_mode &
           static_cast<uint8_t>(MAV_MODE_FLAG::MAV_MODE_FLAG_SAFETY_ARMED)) !=
          0;
      ...
      if (heartbeat.autopilot ==
        ...
        if (!isOffboard && (requested_mode_ != custom) &&
            (custom != previous_mode_)) {
          if (control_request_sent_) {
            // user may have changed modes on us! So we need to honor that and
            // not try and take it back.
            vehicle_state_.controls.offboard = false;
            control_requested_ = false;
            control_request_sent_ = false;

            Utils::log(Utils::stringf(
                "MavLinkVehicle: detected mode change (mode=%d, submode=%d), "
                "will stop trying to do offboard control\n",
                mode, submode));
          }
        }
    ...
    case MavLinkAttitude::kMessageId: {  // MAVLINK_MSG_ID_ATTITUDE:
      ...
      // Utils::logMessage("Received attitude, acc=[%2.2f %2.2f %2.2f]",
    ...
    case MavLinkGlobalPositionInt::
        kMessageId: {  // MAVLINK_MSG_ID_GLOBAL_POSITION_INT:
    ...
    case MavLinkServoOutputRaw::
        kMessageId: {  // MAVLINK_MSG_ID_SERVO_OUTPUT_RAW:
    ...
    case MavLinkHomePosition::kMessageId: {  // MAVLINK_MSG_ID_HOME_POSITION:
      ...
      state_version_++;
      updateReadStats(msg);
      vehicle_state_.home.global_pos.lat =
          static_cast<float>(home.latitude) / 1E7f;
      vehicle_state_.home.global_pos.lon =
          static_cast<float>(home.longitude) / 1E7f;
      vehicle_state_.home.global_pos.alt =
          static_cast<float>(home.altitude) / 1E3f;
    ...
    case MavLinkExtendedSysState::
        kMessageId: {  // MAVLINK_MSG_ID_EXTENDED_SYS_STATE:
      // Provides state for additional features
      // The general system state
      MavLinkExtendedSysState extstatus;
      extstatus.decode(msg);
      bool landed =
          extstatus.landed_state ==
          static_cast<int>(MAV_LANDED_STATE::MAV_LANDED_STATE_ON_GROUND);
      std::lock_guard<std::mutex> guard(state_mutex_);
    ...
    case MavLinkActuatorControlTarget::
        kMessageId: {  // MAVLINK_MSG_ID_ACTUATOR_CONTROL_TARGE
      break;
    }
    ...
    case MavLinkMessageInterval::
        kMessageId: {  // MAVLINK_MSG_ID_MESSAGE_INTERVAL:
      MavLinkMessageInterval msgInt;
      msgInt.decode(msg);
      Utils::log(Utils::stringf(
          "Received message interval for msg=%d, interval = %d",
    ...
    case MavLinkCommandAck::kMessageId: {
      // This one is tricky, we can't do sendCommandAndWaitForAck in this case
      // because it takes too long but we do want to know when we get the ack.
      // So this is async ACK processing!
      ...
      if (ack.command == MavCmdNavGuidedEnable::kCommandId) {
        ...
        if (ackResult == MAV_RESULT::MAV_RESULT_TEMPORARILY_REJECTED) {
          Utils::log(
              "### command MavCmdNavGuidedEnable result: "
          ...
          Utils::log(
              "### command MavCmdNavGuidedEnable result: MAV_RESULT_FAILED");
          vehicle_state_.controls.offboard = false;
        } else if (ackResult == MAV_RESULT::MAV_RESULT_ACCEPTED) {
          Utils::log(
              "### command MavCmdNavGuidedEnableresult: MAV_RESULT_ACCEPTED");
...
void MavLinkVehicleImpl::writeMessage(MavLinkMessageBase& msg,
                                      bool update_stats) {
  sendMessage(msg);
  if (update_stats) {
...
AsyncResult<bool> MavLinkVehicleImpl::takeoff(float z, float pitch, float yaw) {
  // careful here, we are doing a tricky conversion from local coordinates to
  // global coordinates.
...
AsyncResult<bool> MavLinkVehicleImpl::waitForAltitude(float z, float dz,
                                                      float dvz) {
  std::shared_ptr<MavLinkConnection> con = ensureConnection();
  ...
  int subscription =
      con->subscribe([=](std::shared_ptr<MavLinkConnection> connection,
        ...
        unused(connection);
        if (m.msgid ==
            static_cast<uint8_t>(MavLinkLocalPositionNed::kMessageId)) {
...
AsyncResult<bool> MavLinkVehicleImpl::land(float yaw, float lat, float lon,
                                           float altitude) {
  MavCmdNavLand cmd{};
  cmd.AbortAlt = 0;
  cmd.YawAngle = yaw;
...
bool MavLinkVehicleImpl::getRcSwitch(int channel, float threshold) {
  if (threshold < -1 || threshold > 1) {
    auto msg = Utils::stringf(
        "RC channel threshold is out of range, expecting -1 < threshold < 1, "
        "but got %f",
        threshold);
  ...
  if (channel > 0 && channel < 18) {
    ...
    // (invalid) INT16_MAX. Convert it to a floating point number between -1
    // and 1.
...
void MavLinkVehicleImpl::requestControl() {
  // PX4 expects the move commands to happen IMMEDIATELY after this call, so we
  // don't actually request control here until the move commands start
  // happening.
  control_requested_ = true;
}

// return true if user calls requestControl and has not called releaseControl.
bool MavLinkVehicleImpl::hasOffboardControl() { return control_requested_; }
...
void MavLinkVehicleImpl::checkOffboard() {
  ...
  if (control_requested_ && !vehicle_state_.controls.offboard) {
    // Ok, now's the time to actually request it since the caller is about to
    // send MavLinkSetPositionTargetGlobalInt, but PX4 will reject this thinking
    // 'offboard_control_loss_timeout' because we haven't actually sent any
    // offboard messages yet.  I know the PX4 protocol is kind of weird.  So we
    // prime the pump here with some dummy messages that tell the drone to stay
    // where it is, this will reset the 'offboard_control_loss_timeout', then we
    // should be able to get control.
    ...
    Utils::log(
        "MavLinkVehicleImpl::checkOffboard: sending MavCmdNavGuidedEnable \n");
    // Note: we can't wait for ACK here, I've tried it.  The ACK takes too long
    // to get back to us by which time the PX4 times out offboard mode!!
    setMode(static_cast<int>(MAV_MODE_FLAG::MAV_MODE_FLAG_CUSTOM_MODE_ENABLED),
...
void MavLinkVehicleImpl::moveToGlobalPosition(float lat, float lon, float alt,
                                              bool isYaw, float yawOrRate) {
  checkOffboard();
...
AsyncResult<bool> MavLinkVehicleImpl::setMode(int mode, int customMode,
                                              int customSubMode,
                                              bool waitForAck) {
  ...
  if ((vehicle_state_.mode &
       static_cast<int>(MAV_MODE_FLAG::MAV_MODE_FLAG_HIL_ENABLED)) != 0) {
    mode |= static_cast<int>(
        MAV_MODE_FLAG::MAV_MODE_FLAG_HIL_ENABLED);  // must preserve this flag.
  }
  if ((vehicle_state_.mode &
       static_cast<uint8_t>(MAV_MODE_FLAG::MAV_MODE_FLAG_SAFETY_ARMED)) != 0) {
    mode |= static_cast<int>(
        MAV_MODE_FLAG::MAV_MODE_FLAG_SAFETY_ARMED);  // must preserve this flag.
  }
...
AsyncResult<bool> MavLinkVehicleImpl::setPositionHoldMode() {
  return setMode(
      static_cast<int>(MAV_MODE_FLAG::MAV_MODE_FLAG_CUSTOM_MODE_ENABLED),
...
AsyncResult<bool> MavLinkVehicleImpl::setHomePosition(float lat, float lon,
                                                      float alt) {
  MavCmdDoSetHome cmd{};
  cmd.UseCurrent = 0;
  cmd.Latitude = lat;
...
AsyncResult<bool> MavLinkVehicleImpl::setVTOLState(bool fixedWing) {
  Utils::log(Utils::stringf("VTOL state to %s",
                            fixedWing ? "fixed-wing" : "multicopter"));
  MavCmdDoVtolTransition cmd{};
  cmd.State = static_cast<float>(fixedWing ? MAV_VTOL_STATE::MAV_VTOL_STATE_FW
                                           : MAV_VTOL_STATE::MAV_VTOL_STATE_MC);
  cmd.Immediate = 0;
  return sendCommandAndWaitForAck(cmd);
}
...
bool MavLinkVehicleImpl::isLocalControlSupported() {
  ...
  if (!getCapabilities().wait(2000, &cap)) {
    throw std::runtime_error(
        "Two second timeout waiting for response to mavlink command "
...
void MavLinkVehicleImpl::moveByLocalVelocity(float vx, float vy, float vz,
                                             bool isYaw, float yawOrRate) {
  checkOffboard();
...
void MavLinkVehicleImpl::moveByLocalVelocityWithAltHold(float vx, float vy,
                                                        float z, bool isYaw,
                                                        float yawOrRate) {
  checkOffboard();
...
void MavLinkVehicleImpl::moveToLocalPosition(float x, float y, float z,
                                             bool isYaw, float yawOrRate) {
  checkOffboard();
...
bool MavLinkVehicleImpl::isAttitudeControlSupported() {
  ...
  if (!getCapabilities().wait(5000, &cap)) {
    throw std::runtime_error(
        "Five second timeout waiting for response to mavlink command "
        "MAV_CMD_REQUEST_AUTOPILOT_CAPABILITIES");
  }
  return (cap.capabilities &
...
void MavLinkVehicleImpl::moveByAttitude(float roll, float pitch, float yaw,
                                        float rollRate, float pitchRate,
                                        float yawRate, float thrust) {
  checkOffboard();
  ...
  mavlink_euler_to_quaternion(roll, pitch, yaw, msg.q);

  // thrust must be between -1 and 1.
  thrust = static_cast<float>(fmax(-1.0f, fmin(1.0f, thrust)));
  msg.thrust = thrust;
  msg.body_roll_rate = rollRate;
  msg.body_pitch_rate = pitchRate;
  msg.body_yaw_rate = yawRate;
