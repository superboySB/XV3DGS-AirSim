 ...
 public:
  semaphore_impl() {
    ...
    if (this->semaphore == NULL) {
      throw std::runtime_error(Utils::stringf(
          "CreateSemaphore failed with errno=%d!\n", GetLastError()));
    }
  }
  ...
  void post() {
    ...
    if (0 == ReleaseSemaphore(semaphore, 1, &previous)) {
      throw std::runtime_error(Utils::stringf(
          "ReleaseSemaphore failed with errno=%d!\n", GetLastError()));
    }
  }

  // WaitOne indefinitely for one Signal.  If a Signal has already been posted
  // then WaitOne returns immediately decrementing the count so the next WaitOne
  // may block.
  void wait() {
    ...
    if (rc == WAIT_OBJECT_0) {
      ...
      throw std::runtime_error(Utils::stringf(
          "WaitForSingleObjectEx failed with unexpected errno=%d!\n",
          GetLastError()));
    }
  }
  ...
  bool timed_wait(int milliseconds) {
    ...
    if (rc == WAIT_OBJECT_0) {
      ...
      throw std::runtime_error(Utils::stringf(
          "WaitForSingleObjectEx failed with errno=%d!\n", GetLastError()));
    }
    return false;
  }
};
 ...
 public:
  semaphore_impl() {
    ...
    kern_return_t rc = semaphore_create(
        owner, &semaphore, /* policy */ SYNC_POLICY_FIFO, /* value*/ 0);
    if (rc != KERN_SUCCESS) {
      throw std::runtime_error(
  ...
  ~semaphore_impl() { semaphore_destroy(owner, semaphore); }

  void post() {
    kern_return_t rc = semaphore_signal(semaphore);
    if (rc != KERN_SUCCESS) {
      throw std::runtime_error(
          Utils::stringf("OSX semaphore_signal failed with error %d\n", rc));
    }
  }
  ...
  void wait() {
    ...
    if (rc != KERN_SUCCESS) {
      throw std::runtime_error(Utils::stringf(
          "OSX semaphore_wait failed with unexpected error %d\n", rc));
    }
  }
  ...
  bool timed_wait(int milliseconds) {
    ...
    auto absolute = std::chrono::system_clock::now().time_since_epoch() +
                    std::chrono::milliseconds(milliseconds);
    auto seconds = std::chrono::duration_cast<std::chrono::seconds>(absolute);
    absolute = absolute - seconds;
    auto nanoSecondsRemaining =
        std::chrono::duration_cast<std::chrono::nanoseconds>(absolute);
    ...
    switch (rc) {
      ...
      default:
        throw std::runtime_error(Utils::stringf(
            "OSX semaphore_timedwait failed with unexpected error %d", rc));
    }
  }
};
#else                // assume posix
...
class Semaphore::semaphore_impl {
 ...
 public:
  ...
  ~semaphore_impl() { sem_destroy(&semaphore); }

  void post() {
    int status = sem_post(&semaphore);
    if (status < 0) {
      throw std::runtime_error(
          Utils::stringf("sem_post failed with errno=%d!\n", errno));
    }
  }
  ...
  void wait() {
    ...
    if (rc != 0) {
      throw std::runtime_error(
          Utils::stringf("sem_wait failed with unexpected errno=%d!\n", rc));
    }
  }
  ...
  bool timed_wait(int milliseconds) {
    ...
    if (rc != 0) {
      ...
      if (rc == ETIMEDOUT) {
        ...
        throw std::runtime_error(Utils::stringf(
            "sem_timedwait failed with unexpected errno=%d!\n", rc));
      }
    }
    return rc == 0;
  }
...
Semaphore::Semaphore() { impl_.reset(new semaphore_impl()); }

Semaphore::~Semaphore() {}

void Semaphore::wait() { impl_->wait(); }

void Semaphore::post() { impl_->post(); }
