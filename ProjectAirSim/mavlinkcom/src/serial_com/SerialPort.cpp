...
class SerialPort::serialport_impl {
 ...
 public:
  ...
  int connect(const char* portName, int baudRate) {
    ...
    handle = CreateFileW(wide.c_str(), GENERIC_READ | GENERIC_WRITE, 0, 0,
                         OPEN_EXISTING,
    ...
    HRESULT hr = setAttributes(baudRate, parity, dataBits, sb, hs, readTimeout,
                               writeTimeout);
    if (hr != 0) {
      return hr;
    }
    if (!PurgeComm(handle, PURGE_RXCLEAR | PURGE_TXCLEAR) ||
        !SetupComm(handle, readBufferSize, writeBufferSize)) {
      return GetLastError();
    }
    ...
    timeouts.ReadTotalTimeoutConstant =
        (readTimeout == -1 ? MAXDWORD - 1 : (DWORD)readTimeout);

    timeouts.WriteTotalTimeoutMultiplier = 0;
    timeouts.WriteTotalTimeoutConstant =
        (writeTimeout == -1 ? MAXDWORD : (DWORD)writeTimeout);
    ...
    if (hs != Handshake_RequestToSend && hs != Handshake_RequestToSendXonXoff) {
  ...
  int setAttributes(int baudRate, Parity parity, int dataBits, StopBits bits,
                    Handshake hs, int readTimeout, int writeTimeout) {
    DCB dcb;
  ...
  int read(uint8_t* buffer, int bytesToRead) {
    ...
    if (ReadFile(handle, buffer, bytesToRead, &numberOfBytesRead,
                 &readOverlapped)) {
    ...
    if (!GetOverlappedResult(handle, &readOverlapped, &numberOfBytesRead,
                             TRUE)) {
      // return GetLastError();
      return -1;
    }
...
class SerialPort::serialport_impl {
 ...
 public:
  ...
  int connect(const char* portName, int baudRate) {
    ...
    if (setAttributes(baudRate, parity, dataBits, sb, hs, readTimeout,
                      writeTimeout) != 0)
      return -1;
  ...
  int setAttributes(int baudRate, Parity parity, int dataBits,
    ...
    switch (dataBits) {
      ...
      case 8:
        ...
        break;
      default:
        // Utils::logMessage("unsupported data size %d (expecting 5,6,7, or 8)",
        // dataBits);
        return -1;
    }
    ...
    tty.c_lflag &=
        ~(ECHO | ECHONL | ICANON | ISIG |
          IEXTEN);  // no signaling chars, no echo, no canonical processing
    tty.c_oflag &= ~OPOST;
    tty.c_cc[VMIN] = 1;  // read blocks with a minimum of 1 char.
    if (readTimeout != -1) {
      tty.c_cc[VTIME] =
          readTimeout / 100;  // convert milliseconds to deciseconds.
    }
...
SerialPort::SerialPort() { impl_.reset(new SerialPort::serialport_impl()); }

SerialPort::~SerialPort() { impl_->close(); }

bool SerialPort::isClosed() { return impl_->isClosed(); }
