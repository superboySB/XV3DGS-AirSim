...
MavLinkVideoClient::~MavLinkVideoClient() { pImpl = nullptr; }

// image APIs
void MavLinkVideoClient::requestVideo(int camera_id, float every_n_sec,
                                      bool save_locally) {
  auto ptr = dynamic_cast<MavLinkVideoClientImpl*>(pImpl.get());
  ptr->requestVideo(camera_id, every_n_sec, save_locally);
}

// or if you are implementing the client side call this function to get the most
// recent frame. returns false if there is no new frame available.
...
MavLinkVideoServer::~MavLinkVideoServer() { pImpl = nullptr; }
...
void MavLinkVideoServer::sendFrame(uint8_t data[], uint32_t data_size,
                                   uint16_t width, uint16_t height,
                                   uint8_t image_type, uint8_t image_quality) {
  auto ptr = dynamic_cast<MavLinkVideoServerImpl*>(pImpl.get());
