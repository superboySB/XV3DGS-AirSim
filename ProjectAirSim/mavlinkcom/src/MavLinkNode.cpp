...
MavLinkNode::MavLinkNode() : pImpl(nullptr) {}
...
// already doing it.
void MavLinkNode::startHeartbeat(const MavLinkHeartbeat& heartbeat) {
  pImpl->startHeartbeat(heartbeat);
}

// stop listening to the connection.
void MavLinkNode::close() { pImpl->close(); }
...
AsyncResult<MavLinkParameter> MavLinkNode::getParameter(
...
void MavLinkNode::sendOneHeartbeat() { return pImpl->sendOneHeartbeat(); }
...
int MavLinkNode::getLocalSystemId() { return pImpl->getLocalSystemId(); }
int MavLinkNode::getLocalComponentId() { return pImpl->getLocalComponentId(); }
int MavLinkNode::getTargetSystemId() { return pImpl->getTargetSystemId(); }
...
void MavLinkNode::sendMessage(MavLinkMessage& msg) { pImpl->sendMessage(msg); }

// send a command to the remote node
void MavLinkNode::sendCommand(MavLinkCommand& cmd) { pImpl->sendCommand(cmd); }
