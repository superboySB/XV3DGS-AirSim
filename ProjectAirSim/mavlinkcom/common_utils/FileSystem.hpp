...
// This defines a default folder name for all the files created by AirLib so
// they are all gathered nicely in one place in the user's documents folder.
...
class FileSystem {
 ...
 public:
  ...
  static std::string resolve(std::string path, std::string rel) {
    ...
    for (auto iter = vector.begin(); iter != vector.end(); iter++) {
      if (*iter == "..") {
        ...
        if (currentPath.size() == 0 ||
            currentPath[currentPath.size() - 1] != kPathSeparator) {
    ...
    // PX4 doesn't like relative names (it horks the board, no more responses
    // after that)
  ...
  static std::string combine(const std::string parentFolder,
  ...
  static std::string getLogFileNamePath(std::string prefix, std::string suffix,
    ...
    filename_ss << ensureFolder(fullPath) << kPathSeparator << prefix << suffix
                << timestamp << extension;
...
#ifdef _WIN32
    // WIN32 will create the wrong file names if we don't first convert them to
    // UTF-16.
    std::wstring_convert<std::codecvt_utf8<wchar_t>, wchar_t> converter;
