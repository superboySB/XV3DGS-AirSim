...
#ifndef M_PIl
#define M_PIl \
  static_cast<long double>(3.1415926535897932384626433832795028841972)
#endif
...
/*
    ...
    by dropping in common_utils.hpp. Therefore there should not be any
   dependency in the code below other than STL. The code should be able to
   compilable on all major platforms.
*/

#ifndef _MSC_VER
__attribute__((__format__(__printf__, 1, 0))) static int _vscprintf(
...
class Utils {
 ...
 public:
  ...
  static const string printRange(Range&& range, const string& delim = ", ",
                                 const string& prefix = "(",
                                 const string& suffix = ")") {
    return printRange(std::begin(range), std::end(range), delim, prefix,
                      suffix);
  }
  template <typename Iterator>
  static const string printRange(Iterator start, Iterator last,
                                 const string& delim = ", ",
                                 const string& prefix = "(",
                                 const string& suffix = ")") {
    stringstream ss;
  ...
  static std::vector<std::string> split(const string& s, const char* splitChars,
                                        int numSplitChars) {
    auto start = s.begin();
  ...
  // split a line into tokens using any of the given separators as token
  // separators. this method also understands quoted string literals (either
  // single or double quotes) and returns the quoted value without the quotes,
  // and this value can contain separators.
  ...
  static typename std::enable_if<type_utils::is_container<Container>::value,
                                 void>::type
  ...
  static const char* to_string(time_point<steady_clock> t) {
    time_t tt = system_clock::to_time_t(
        std::chrono::time_point_cast<system_clock::duration>(
            system_clock::now() + (t - steady_clock::now())));
    return ctime(&tt);
  }

  static time_point<system_clock> now() { return system_clock::now(); }
  ...
  static double getTimeSinceEpochSecs(
      std::chrono::high_resolution_clock::time_point* t = nullptr) {
    using Clock = std::chrono::high_resolution_clock;
    return std::chrono::duration<double>(
               (t != nullptr ? *t : Clock::now()).time_since_epoch())
        .count();
  }
  static uint64_t getTimeSinceEpochNanos(
      std::chrono::high_resolution_clock::time_point* t = nullptr) {
    using Clock = std::chrono::high_resolution_clock;
    return static_cast<uint64_t>(
        (t != nullptr ? *t : Clock::now()).time_since_epoch().count());
  }
  ...
  // for example, default tolerance may not work if you are comparing double
  // with float
  template <typename TReal>
  ...
  static bool isWithinPrecisionInterval(TReal a, TReal b,
    ...
    TReal min_a =
        a - (a - std::nextafter(a, std::numeric_limits<TReal>::lowest())) *
                interval_size;
    TReal max_a =
        a + (std::nextafter(a, std::numeric_limits<TReal>::max()) - a) *
                interval_size;
