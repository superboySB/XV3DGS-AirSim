...
class EnvActor::Impl : public ActorImpl {
 public:
  ...
  void UpdateKinematics(TimeSec curr_time);
...
bool EnvActor::SetLinkRotationAngle(const std::string& link_name,
                                    const float angle_deg) {
  return static_cast<EnvActor::Impl*>(pimpl_.get())
      ->SetLinkRotationAngle(link_name, angle_deg);
}
...
void EnvActor::UpdateKinematics(TimeSec curr_time) {
  return static_cast<EnvActor::Impl*>(pimpl_.get())
...
EnvActor::Impl::Impl(const std::string& id, const Pose& origin,
                     const Logger& logger, const TopicManager& topic_manager,
                     const std::string& parent_topic_path,
                     const ServiceManager& service_manager,
                     const StateManager& state_manager)
...
void EnvActor::Impl::UpdateKinematics(TimeSec curr_time) {
  if (traj_ptr_) {
    traj_ptr_->KinematicsUpdate(curr_time, traj_kinematics_);
